...
thanks: "Code and select data available at: https://github.com/InessaDeAngelis/Torrential_Twitter_Paper "
date: "today"
date-format: "long"
...
# This code fixed the gap issue! https://stackoverflow.com/questions/67365583/extra-white-space-after-knitting-in-rmarkdown
...
\end{aligned}
$$ Where $y_i$ is whether a Tweet contained harassment, $\pi_i = \mbox{Pr}(y_i=1)$, and $\alpha^{\mbox{account}}$, $\alpha^{\mbox{politician}}$, and $\alpha^{\mbox{verified}}$ are the effect of the type of account a user has, which politician they are replying to, and whether their account is verified. The $a[i]$, $p[i]$, and $v[i]$ refer to which type of account, which politician, and whether a user is verified. $A$, $P$, and $V$ are the total number of types of accounts, politicians, and verification options, respectively. The Positive and Neutral categories on my seven-point scale make up the value of 0 and all remaining categories (Questioning authority through Hate speech) make up the value of 1.  The binary response variable, $y_i$, is called `harassment_binary` and has a value of 0 if a Tweet does not contain harassment and a value of 1 if it contains harassment. 
...
```
@tbl-modsummary presents the estimates generated by the model. For ease of analysis, @tbl-marginal displays the estimates as predictions in a table and @fig-plotted2 shows the predictions graphed.
...
ggplot(harassment_model_predictions, aes(verified, estimate)) +
  geom_jitter(aes(colour = factor(name_of_politician))) +
  facet_wrap(harassment_binary ~type_of_account) +
  labs(
    x = "Whether an account is verified",
  ...
  scale_colour_manual(values = c("darkblue", "darkcyan", "yellow"))
